...
def main():
    ...
    report_lines = [
        ...
        "",
        "## 5. Main Results",
        f"On the easy split, the current placeholders are: MLP success `{metric_text(easy, 'mlp', 'success_rate')}`, GNN success `{metric_text(easy, 'gnn', 'success_rate')}`, and Predictive success `{metric_text(easy, 'predictive', 'success_rate')}`.",
        f"On the medium split, the current placeholders are: MLP success `{metric_text(medium, 'mlp', 'success_rate')}`, GNN success `{metric_text(medium, 'gnn', 'success_rate')}`, and Predictive success `{metric_text(medium, 'predictive', 'success_rate')}`.",
        f"On the hard split, the current placeholders are: MLP success `{metric_text(hard, 'mlp', 'success_rate')}`, GNN success `{metric_text(hard, 'gnn', 'success_rate')}`, and Predictive success `{metric_text(hard, 'predictive', 'success_rate')}`.",
        "",
        f"Insert the pivot table from `{report_dir / 'main_results_pivot.csv'}` and the figures from `{figures_dir / 'success_rate.svg'}`, `{figures_dir / 'collision_rate.svg'}`, and `{figures_dir / 'path_efficiency.svg'}`. Replace these placeholder sentences with the strongest supported claim after final runs.",
        "",
    ...
    slide_lines = [
        ...
        f"- Use `{figures_dir / 'success_rate.svg'}` and `{report_dir / 'main_results_pivot.csv'}`.",
        f"- Easy placeholder: predictive success `{metric_text(easy, 'predictive', 'success_rate')}`.",
        f"- Hard placeholder: predictive success `{metric_text(hard, 'predictive', 'success_rate')}`.",
